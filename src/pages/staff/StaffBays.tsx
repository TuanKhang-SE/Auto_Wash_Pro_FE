...
function getServiceNames(item: BookingItem) {
  ...
  return names.length > 0 ? names.join(", ") : "Chưa cập nhật dịch vụ";
}

function getVehicleName(item: BookingItem) {
  const name = [item.Vehicles?.Brand, item.Vehicles?.Model]
    .filter(Boolean)
    .join(" ")
    .trim();
...
function getStaffBranchId(token: string) {
  try {
    ...
    if (storedUser) {
      ...
      const branchId = Number(user.branchId ?? user.BranchID);
    ...
    // Nếu localStorage cũ bị lỗi, tiếp tục đọc BranchID từ JWT.
  }
...
function StaffBays() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    if (!token) {
      setMessage("Bạn cần đăng nhập bằng tài khoản nhân viên.");
      setLoading(false);
      return;
    }

    const branchId = getStaffBranchId(token);
    ...
    try {
      ...
      const [branchResponse, slotResponse, bookingResponse] = await Promise.all([
        axiosClient.get(`/api/branches/${branchId}`, {
          params: { refreshTime: Date.now() },
        }),
      ...
      console.log("LOAD STAFF BAYS ERROR:", error);
  ...
  const loadMonthData = useCallback(async () => {
    if (monthRequestRunningRef.current) return;
    ...
    if (!branchId) {
      setMessage("Không tìm thấy chi nhánh của nhân viên.");
      return;
    }
    ...
    try {
            ...
            try {
              ...
              return {
                ...
                maxStaffCount: slots.reduce(
                  (max, slot) => Math.max(max, Number(slot.StaffCount || 0)),
                  0
                ),
                hasSchedule: slots.length > 0,
              };
            } catch (error) {
              console.log(`LOAD MONTH DAY ${dateValue} ERROR:`, error);
      ...
      console.log("LOAD STAFF BAYS MONTH ERROR:", error);
      setMessage(getErrorMessage(error));
    } finally {
  ...
  const activeWashes = useMemo(
    () =>
      allItems
        ...
        .sort((a, b) =>
          String(a.item.WashStartAt || "").localeCompare(
            String(b.item.WashStartAt || "")
          )
        ),
    [allItems]
  );
  ...
  const checkedInCount = useMemo(
    () => allItems.filter(({ item }) => item.Status === "CheckedIn").length,
    [allItems]
  );
  ...
  const totalBays = Math.max(0, Number(slotData?.TotalWashBays || 0));
  const occupiedCount = Math.min(activeWashes.length, totalBays);
  const availableCount = Math.max(0, totalBays - occupiedCount);
  const overflowCount = Math.max(0, activeWashes.length - totalBays);
  const usagePercent =
    totalBays > 0 ? Math.round((occupiedCount / totalBays) * 100) : 0;

  const bays = useMemo<BayView[]>(
    () =>
      Array.from({ length: totalBays }, (_, index) => ({
        number: index + 1,
        status: index < activeWashes.length ? "Occupied" : "Available",
        activeWash: activeWashes[index],
      })),
    [activeWashes, totalBays]
  );

  /*
   * Hiển thị toàn bộ slot mà API trả về.
   * Không dùng slice(0, 8), nên danh sách có thể hiển thị
   * đầy đủ tới giờ kết thúc ca, ví dụ 22:00.
   */
      ...
      <div className="flex min-h-[420px] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm font-medium text-slate-500">
            Đang tải dữ liệu trạm và slot từ hệ thống...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Trạm Rửa Xe</h1>
          <p className="mt-2 text-slate-500">
            Theo dõi trạm đang sử dụng và các slot còn trống của chi nhánh theo dữ liệu API.
          </p>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-2 text-sm text-blue-100">
                <Clock3 size={17} />
                <span>
                  Hoạt động từ {formatTime(branch?.OpenTime)} đến {formatTime(branch?.CloseTime)}
                </span>
              </div>
            </div>
          </div>

          <div className="grid w-full gap-3 sm:grid-cols-2 xl:w-[610px]">
            <div className="rounded-xl bg-white/10 p-4 backdrop-blur-sm sm:col-span-2">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <p className="text-xs font-medium text-blue-100">Bộ lọc theo dõi</p>

                  <div className="mt-2 inline-flex rounded-lg bg-white/10 p-1">
                    <button
                      type="button"
                      onClick={() => setViewMode("day")}
                      className={`rounded-md px-3 py-1.5 text-xs font-bold transition ${viewMode === "day"
                    ...
                    <button
                      ...
                      className={`rounded-md px-3 py-1.5 text-xs font-bold transition ${viewMode === "month"
                          ? "bg-white text-blue-700 shadow-sm"
                          : "text-white hover:bg-white/10"
                        }`}
                    >
                ...
                <div className="min-w-0 lg:w-56">
                  <label
                    htmlFor={viewMode === "day" ? "selectedDate" : "selectedMonth"}
            ...
            <div className="rounded-xl bg-white/10 p-4 backdrop-blur-sm">
              <p className="text-xs font-medium text-blue-100">Chờ vào trạm</p>
              <p className="mt-1 text-xl font-bold">
                {viewMode === "day" ? checkedInCount : monthlyTotals.booked}
              </p>
              <p className="mt-1 text-[11px] text-blue-100">
                {viewMode === "day" ? "Xe đã check-in" : "Lượt đã đặt trong tháng"}
              </p>
            </div>

            <div className="rounded-xl bg-white/10 p-4 backdrop-blur-sm">
              <p className="text-xs font-medium text-blue-100">
                {viewMode === "day" ? "Chờ check-in" : "Ngày có lịch"}
              </p>
              <p className="mt-1 text-xl font-bold">
                {viewMode === "day"
                  ? waitingCount
              ...
              <p className="mt-1 text-[11px] text-blue-100">
                {viewMode === "day"
                  ? "Xe chưa check-in"
                  : `Trong ${formatMonthLabel(selectedMonth)}`}
              </p>
            </div>

            <p className="text-xs text-blue-100 sm:col-span-2">
              Cập nhật gần nhất: {lastUpdated ? lastUpdated.toLocaleTimeString("vi-VN") : "--:--:--"}
            </p>
          </div>
        </div>
      </section>
      ...
      {viewMode === "month" ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Bấm vào một ngày để mở chi tiết trạm và slot của ngày đó.
                </p>
              </div>
            </div>
            ...
            {monthlyData.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-28 rounded-xl border p-2 text-left transition hover:-translate-y-0.5 hover:shadow-md ${day.failed
                      ? "border-red-200 bg-red-50"
                      : day.hasSchedule
                        ? "border-emerald-200 bg-emerald-50"
                        : "border-slate-200 bg-slate-50"
                    } ${isToday ? "ring-2 ring-blue-400" : ""} ${isSelected ? "shadow-md" : ""
                    }`}
                >
                  ...
                  {day.failed ? (
                    ...
                    <div className="mt-3 space-y-1 text-[11px]">
                      ...
                      <p className="text-emerald-700">Trống: {day.available}</p>
          ...
          <p className="mt-4 text-xs leading-5 text-slate-500">
            Chế độ tháng gọi API available-slots cho từng ngày. Các ngày đã qua có thể không còn hiển thị slot vì Backend bỏ qua khung giờ trong quá khứ.
          </p>
        </section>
      ) : (
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-2 text-3xl font-bold text-slate-800">{totalBays}</p>
                </div>
              ...
              <p className="mt-3 text-xs text-slate-400">
                Lấy từ TotalWashBays của API slot
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Trạm đang trống</p>
              ...
              <p className="mt-3 text-xs text-emerald-600">Có thể nhận xe vào rửa</p>
            </div>

            <div className="rounded-2xl border border-blue-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-500">Trạm đang sử dụng</p>
              ...
              <p className="mt-3 text-xs text-blue-600">
                Công suất hiện tại {usagePercent}%
              </p>
            </div>
          </section>
          ...
          {overflowCount > 0 && (
            <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
              <AlertCircle className="mt-0.5 shrink-0" size={18} />
              <span>
                Có {overflowCount} xe đang ở trạng thái Đang rửa vượt quá TotalWashBays. Hãy kiểm tra lại dữ liệu booking hoặc cấu hình chi nhánh.
              </span>
            </div>
          )}
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {bays.length === 0 ? (
                <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center">
                  <AlertCircle className="mx-auto text-amber-500" size={30} />
                  <p className="mt-3 font-semibold text-slate-700">
                    Chưa có cấu hình số trạm
                  </p>
                  <p className="mt-1 text-sm text-slate-500">
                    API không trả về TotalWashBays cho chi nhánh này.
                  </p>
                </div>
              ) : (
                <div className="grid gap-4 md:grid-cols-2">
                  {bays.map((bay) => {
                    const item = bay.activeWash?.item;
                    const booking = bay.activeWash?.booking;

                    return (
                      <article
                        key={bay.number}
                        className={`rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-md ${bay.status === "Available"
                            ? "border-emerald-200 bg-emerald-50"
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-bold text-slate-800">
                                Trạm {String(bay.number).padStart(2, "0")}
                              </h3>
                              <p
                                className={`mt-0.5 text-xs font-semibold ${bay.status === "Available"
                                    ? "text-emerald-700"
                                    : "text-blue-700"
                                  }`}
                              >
                          ...
                          <span
                            className={`mt-1 h-3 w-3 shrink-0 rounded-full ${bay.status === "Available"
                          ...
                          <div className="mt-4 space-y-3 rounded-xl border border-blue-200 bg-white/80 p-4">
                            ...
                            <div className="border-t border-slate-200 pt-3">
                              ...
                              <p className="text-sm font-semibold text-blue-700">
                                {item ? getServiceNames(item) : "Chưa cập nhật"}
                              </p>
                            </div>

                            <div className="grid grid-cols-2 gap-3 border-t border-slate-200 pt-3">
                              <div>
                                <p className="text-xs text-slate-500">Bắt đầu rửa</p>
                                <p className="text-sm font-bold text-slate-700">
                                  {formatTime(item?.WashStartAt)}
                                </p>
                              </div>
                              <div>
                                <p className="text-xs text-slate-500">Dự kiến theo slot</p>
                                <p className="text-sm font-bold text-blue-700">
                                  {addMinutesToTime(
                                    item?.WashStartAt,
                                    Number(slotData?.SlotDuration || 30)
                                  )}
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-700">
                              <p className="font-semibold">
                                {booking?.BookingCode || "Chưa có mã booking"}
                              </p>
                              <p className="mt-1">
            ...
            <aside className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Hiển thị toàn bộ slot API trả về tới cuối ca
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-5 max-h-[720px] space-y-3 overflow-y-auto pr-1">
                  {upcomingSlots.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-slate-300 p-5 text-center text-sm text-slate-500">
                      Không còn slot khả dụng trong ngày hoặc chưa có lịch làm việc.
                    </div>
                  ) : (
                    upcomingSlots.map((slot) => {
                      const total = Math.max(0, Number(slot.MaxCapacity || totalBays));
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
                            <div
                              className={`h-full rounded-full ${percentage >= 50
                                  ? "bg-emerald-500"
                                  : percentage > 0
                                    ? "bg-amber-500"
                                    : "bg-red-500"
                          ...
                          <div className="mt-2 flex items-center justify-between text-xs text-slate-500">
                            <span>
                              Đã đặt {slot.Booked}/{total}
                            </span>
                            <span className="flex items-center gap-1">
                              <Users size={13} /> {slot.StaffCount} nhân viên
                            </span>
                          </div>
              ...
              <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 p-5 text-white shadow-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm text-slate-300">Tình trạng hiện tại</p>
                ...
                <div className="mt-5 rounded-xl bg-white/5 p-4">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-300">Công suất sử dụng</span>
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-white/5 p-3">
                    <p className="text-xs text-slate-400">Đang trống</p>
                    <p className="mt-1 text-xl font-bold text-emerald-400">
                      {availableCount}
                    </p>
                  </div>
                  <div className="rounded-xl bg-white/5 p-3">
                    <p className="text-xs text-slate-400">Đang sử dụng</p>
                ...
                <p className="mt-4 text-xs leading-5 text-slate-400">
                  Trang tự tải lại sau mỗi 5 giây. Số trạm lấy từ cấu hình chi nhánh; xe đang rửa lấy từ BookingItems có trạng thái InProgress.
                </p>
              </div>
            </aside>
          </section>
